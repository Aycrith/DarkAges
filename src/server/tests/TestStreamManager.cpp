...
TEST_CASE("StreamManager XADD with live Redis", "[foundation][database][!mayfail]") {
    ...
    SECTION("XADD with explicit ID") {
        ...
        std::unordered_map<std::string, std::string> fields = {{"data", "test_value"}};
...
TEST_CASE("StreamManager XREAD with live Redis", "[foundation][database][!mayfail]") {
    ...
    SECTION("XREAD with count limits results") {
        for (int i = 0; i < 3; ++i) {
            bool fired = false;
            std::unordered_map<std::string, std::string> fields = {{"index", std::to_string(i)}};
            redis.xadd("test:stream:count", "*", fields, [&](const AsyncResult<std::string>&) { fired = true; });
            redis.update();
            REQUIRE(fired);
        }
    ...
    SECTION("Multiple field-value pairs preserved through XADD/XREAD") {
        ...
        redis.xadd("test:stream:fields", "*", fields, [&](const AsyncResult<std::string>&) { addFired = true; });
...
TEST_CASE("StreamManager disconnected behavior (Redis available, server down)", "[foundation][database]") {
    ...
    SECTION("Multiple disconnected operations each fire error callbacks") {
        ...
        redis.xadd("stream1", "*", fields, [&](const AsyncResult<std::string>&) { callbackCount++; });
        redis.xadd("stream2", "*", fields, [&](const AsyncResult<std::string>&) { callbackCount++; });
        redis.xread("stream3", "0-0", [&](const AsyncResult<std::vector<StreamEntry>>&) { callbackCount++; });
